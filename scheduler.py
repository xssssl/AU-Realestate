...
class Scheduler(object):
    ...
    def result_median_price(self):
        sql_insert = 'INSERT INTO median_price_suburb_copy_sample (postcodes_id, house_buy, house_buy_update_date, house_buy_2br, house_buy_3br, house_buy_4br, ' \
        ...
        sql_update = 'UPDATE median_price_suburb_copy_sample SET  house_buy=%s, house_buy_update_date=%s, house_buy_2br=%s, house_buy_3br=%s, house_buy_4br=%s, ' \
                                                                 'house_rent=%s, house_rent_update_date=%s, house_rent_2br=%s, house_rent_3br=%s, house_rent_4br=%s, ' \
                                                                 'unit_buy=%s, unit_buy_update_date=%s, unit_buy_2br=%s, unit_buy_3br=%s, unit_buy_4br=%s,' \
                                                                 'unit_rent=%s, unit_rent_update_date=%s, unit_rent_2br=%s, unit_rent_3br=%s, unit_rent_4br=%s,' \
                                                                 'trend=%s WHERE postcodes_id=%s'
        #sql_select = 'SELECT * FROM median_price_suburb_copy_sample WHERE postcodes_id=%s'
        ...
        for _ in range(self.result_queue_qsize):
            ...
            #record = self.dbwrapper.db_fetchall(sql_select, [self.median_price_sub.postcodes_id, ])
            r = self.dbwrapper.db_execute(sql_update, [self.median_price_sub.house_buy, self.median_price_sub.house_buy_update_date, self.median_price_sub.house_buy_2br, self.median_price_sub.house_buy_3br, self.median_price_sub.house_buy_4br,
                                                        ...
                                                        self.median_price_sub.trend, self.median_price_sub.postcodes_id])
    ...
    def rev_result(self):
        ...
        if self.result_queue_maxsize >= 64 and self.result_queue_qsize < round(0.4 * self.result_queue_maxsize):
            return 'abondon'
        else:
            return 'done'
